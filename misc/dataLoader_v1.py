...
class train(data.Dataset):  # torch wrapper
    ...
    def __getitem__(self, index):
        ...
        img_id = self.img_info[index]
        image_features = self.hdf_reader[img_id]
        # image_features = torch.tensor(image_features)
        img = torch.from_numpy(image_features)
        # get the history
        ...
        for i in range(self.rnd):
            ...
            opt_ids = np.delete(opt_ids, random.choice(range(0,len(opt_ids))), 0)
