...
class _netG(nn.Module):
    def __init__(self, args):
        ...
        self.rnn = getattr(nn, 'LSTM')(self.ninp, self.nhid, self.nlayers, bidirectional=True, dropout=self.dropout, batch_first=True)

        self.rnn_type = 'LSTM'

        self.decoder =SimpleClassifier(self.nhid, self.nhid*2 , args.vocab_size, self.dropout)
        self.d = args.dropout
        self.beta = 3
    ...
    def forward(self, emb, hidden):
        ...
        output, hidden1 = self.rnn(emb, hidden)
        output = output[:,:,:self.nhid]
        output = F.dropout(output, self.d, training=self.training)
        decoded = self.decoder(output.contiguous().view(emb.size(0)*emb.size(1), self.nhid))
        logprob = F.log_softmax(self.beta * decoded, 1)

        return logprob.view(emb.size(0), emb.size(1), -1), hidden1
