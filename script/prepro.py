...
def create_mats(data, ques_idx, ans_idx, cap_idx, params, test_type=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-input_json_test',default='../data/visdial_1.0_test.json', help='Input `val` json file')
    # Output files
    parser.add_argument('-output_json', default='../data/visdial_params_v1.0.json', help='Output json file')
    parser.add_argument('-output_h5', default='../data/visdial_data_v1.0.h5', help='Output hdf5 file')
    # Options
    ...
    itow = index2word  # a 1-indexed vocab translation table
    wtoi = word2index  # inverse table
    ...
    opt_test, opt_list_test, opt_len_test, ans_index_test, image_list_test = \
        create_mats(data_test, ques_idx_test, ans_idx_test, cap_idx_test, args, test_type=True)
    ...
    f.create_dataset('ans_index_test', dtype='uint32', data=ans_index_test)
    ...
    out['wtoi'] = wtoi
    out['img_train'] = get_image_ids(data_train)
    out['img_val'] = get_image_ids(data_val)
    out['img_test'] = get_image_ids(data_test)
